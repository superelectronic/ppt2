...
graph TD
    ...
    StartIcon1 -->|XML is more flexible and extensible, with the ability to store hierarchical data better than JSON.| StartBox[Flexibility and Extensibility]
    StartIcon2 -->|JSON is concise and readable due to its simplified syntax, easier to parse and edit.| MiddleBox[Conciseness and Readability]
    StartIcon3 -->|While many modern APIs use JSON, XML still has widespread use in industries that demand backwards compatibility.| EndBox[Adoption in Modern APIs and Technologies]
----
